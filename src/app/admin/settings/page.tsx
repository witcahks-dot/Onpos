  ...
  return (
    <div className="space-y-6 max-w-4xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6 text-xs">
        ...
        {/* ACTIVE THEME SELECTOR (TEMA 1 VS TEMA 2) */}
        <div className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-200 shadow-sm space-y-5">
          <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
            <div>
              <h3 className="font-black text-slate-900 text-sm flex items-center gap-2">
                <Layout className="w-4 h-4 text-blue-600" />
                <span>0. Aktif Web Sitesi Teması Seçimi</span>
              </h3>
              <p className="text-[11px] text-slate-500 mt-0.5">Sitenizin ön yüz tasarımını tek tıkla değiştirebilirsiniz. Tüm CMS ürün ve içerik verileriniz iki temada da ortak çalışır.</p>
            </div>
            <span className="text-[10px] font-bold uppercase bg-emerald-50 text-emerald-700 px-3 py-1 rounded-full border border-emerald-200">
              ÇOKLU TEMA (MULTI-THEME)
            </span>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {/* TEMA 2: Minimalist Fintech Demo */}
            <div
              onClick={() => setForm({ ...form, activeTheme: 'theme2', selectedTheme: 'theme2' })}
              className={`p-5 rounded-2xl border-2 cursor-pointer transition-all relative flex flex-col justify-between ${
                (form.selectedTheme === 'theme2' || form.activeTheme === 'theme2' || !form.selectedTheme)
                  ? 'border-blue-600 bg-blue-50/40 ring-4 ring-blue-600/10 shadow-md'
                  : 'border-slate-200 bg-white hover:border-slate-300'
              }`}
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-slate-900 text-white">
                    <span>✨ YENİ DEMO TASARIM</span>
                  </div>
                  {(form.selectedTheme === 'theme2' || form.activeTheme === 'theme2' || !form.selectedTheme) && (
                ...
                <h4 className="font-black text-slate-900 text-sm pt-1">Tema 2: Minimalist Fintech & SaaS</h4>
                <p className="text-[11px] text-slate-600 leading-relaxed">
                  Asimetrik Hero, yüzen canlı grafik kartları, Bento Grid istatistikler, 2 sütunlu minimalist S.S.S ve modern hap (pill) buton yapısı.
                </p>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between text-[10px] font-bold text-blue-600">
                <span>Önerilen Modern Tasarım</span>
                <span className="font-mono text-slate-400">/?theme=theme2</span>
              </div>
            </div>

            {/* TEMA 1: Klasik Kurumsal POS */}
            <div
              onClick={() => setForm({ ...form, activeTheme: 'theme1', selectedTheme: 'theme1' })}
              className={`p-5 rounded-2xl border-2 cursor-pointer transition-all relative flex flex-col justify-between ${
                (form.selectedTheme === 'theme1' || form.activeTheme === 'theme1')
                  ? 'border-blue-600 bg-blue-50/40 ring-4 ring-blue-600/10 shadow-md'
                  : 'border-slate-200 bg-white hover:border-slate-300'
              }`}
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase bg-blue-100 text-blue-800">
                    <span>KLASİK KURUMSAL</span>
                  </div>
                  {(form.selectedTheme === 'theme1' || form.activeTheme === 'theme1') && (
                    <div className="w-5 h-5 rounded-full bg-blue-600 text-white flex items-center justify-center">
                      <Check className="w-3.5 h-3.5 stroke-[3]" />
                ...
                <h4 className="font-black text-slate-900 text-sm pt-1">Tema 1: Klasik Kurumsal POS</h4>
                <p className="text-[11px] text-slate-600 leading-relaxed">
                  3D döner POS cihaz sahnesi, geniş ürün filtreleme vitrini, kurumsal mavi aksanlar ve klasik modül sıralaması.
                </p>
              </div>
              <div className="mt-4 pt-3 border-t border-slate-200/60 flex items-center justify-between text-[10px] font-bold text-slate-500">
                <span>Mevcut Klasik Tema</span>
                <span className="font-mono text-slate-400">/?theme=theme1</span>
              </div>
            </div>
          </div>
        </div>
