...
export default function FintechSolutionsSection() {
  const { solutions } = useCMSStore();

  const currentSolution = solutions && solutions.length > 0 ? solutions[0] : null;
  const title = currentSolution?.title || 'Power Your Sales With Frictionless Payments';
  const description = currentSolution?.shortDesc || 'Deliver A Frictionless Buying Experience With Secure, Responsive, And Fully Integrated Payment Tools. İster mağazada ister sahada anında tahsilat.';
  const points = currentSolution?.features && currentSolution.features.length > 0
    ? currentSolution.features
    : [
        'Real-Time Payment Tracking',
        'Accept Payments Quickly And Securely',
        'Effortless Integration With Your Platform & Yazarkasa',
      ];
    ...
    <section className="py-20 sm:py-28 bg-[#fbfbfe] border-b border-slate-100/90 overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          {/* Left Column: Dynamic Solutions Headline & Features */}
          <div className="lg:col-span-6 space-y-6 text-center lg:text-left">
            ...
            <div className="pt-3 flex justify-center lg:justify-start">
              ...
              >
                <span>Create Account & Çözümleri Gör</span>
              </Link>
            </div>
