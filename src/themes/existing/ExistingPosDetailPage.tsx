...
export default function ExistingPosDetailPage({ product }: { product: PosProduct }) {
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                {relatedProducts.map(rel => (
                  <div key={rel.id} className="bg-white rounded-3xl border border-slate-200/90 p-6 flex flex-col justify-between hover:shadow-lg transition-all">
                    <img src={resolveImageUrl(rel.images?.[0], 'pos')} alt={rel.name} className="h-44 object-contain mx-auto mb-4" />
